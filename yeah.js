...
movement_tl
  .from("svg", { opacity: 0, duration: 0.1 }, 0)
  .from(
    "svg",
    {
      xPercent: 50,
...
exit_tl.to("svg", { opacity: 0, duration: 0.2 });
exit_tl.to("#overlay", { opacity: 0, duration: 0.1 });

// Main Timeline
const master_tl = gsap.timeline({ repeat: 0, repeatDelay: 1 });
master_tl
  .add(movement_tl, 0)
  .add(n_tl, 0.7)
  .add(e_tl, 0.8)
